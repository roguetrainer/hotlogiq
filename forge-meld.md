...
Gibbs weight at any β. Its non-trivial phase $e^{i\pi/4}$ is purely imaginary in
origin: it is what the Wick rotation $\beta \to it$ introduces and the real axis
cannot supply. In ISA terms, the T-gate is a TWIST opcode carrying a complex phase
that is invisible to the Forge regime.

The Clifford group (Hadamard, CNOT, S-gate) is generated by Meld ISA programmes
over the associative SU(2) sector — SPLIT, SPLAT, TWIST, and BIND at the $j=1/2$
rung. It is efficiently classically simulable (Gottesman-Knill). Adding the T-gate
breaks classical simulability and achieves universal quantum computation, still
within the associative SU(2) sector. **Octonions are not required.**
...
qubit universality. The two levels are:
